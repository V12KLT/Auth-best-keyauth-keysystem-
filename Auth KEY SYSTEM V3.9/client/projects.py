...
def select_project():
    ...
    try:
        ...
        for proj in projects.split("|"):
            if ":" in proj:
                ...
                if len(parts) >= 2:
                    ...
                    keys = int(parts[3]) if len(parts) >= 4 and parts[3].isdigit() else 0
                    active = int(parts[4]) if len(parts) >= 5 and parts[4].isdigit() else 0
                    project_list.append((name, pid))
                    meta_list.append((keys, active))
        ...
        for i, (name, pid) in enumerate(project_list, 1):
            keys, active = meta_list[i - 1]
            tail = f"  {Fore.LIGHTBLACK_EX}({keys} key{'s' if keys != 1 else ''}, {active} active)"
            print(f"{Fore.CYAN}  [{Fore.WHITE}{i}{Fore.CYAN}] ➤ {Fore.WHITE}{name}{tail}{Fore.RESET}")
...
def delete_project_ui():
    ...
    for proj in projects.split("|"):
        if ":" in proj:
            parts = proj.split(":")
            if len(parts) >= 2:
                project_list.append((parts[0], parts[1]))
