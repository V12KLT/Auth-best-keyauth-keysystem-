...
def check_and_update():
    ...
    print(f"{Fore.CYAN}Checking for updates...{Fore.RESET}")
    ...
    try:
        ...
        if update_info.get("update_available"):
            ...
            print(f"\n{Fore.YELLOW}Update available: v{CLIENT_VERSION} -> v{new_version}{Fore.RESET}")
            
            choice = input(f"{Fore.CYAN}Download and install update? (y/n): {Fore.WHITE}").lower()
            if choice == "y":
                print(f"{Fore.CYAN}Downloading update...{Fore.RESET}")
                success, result = perform_update()
                if success:
                    print(f"{Fore.GREEN}Update installed! Restarting...{Fore.RESET}")
                    time.sleep(2)
                    if result == "EXE":
                        os._exit(0)
                    else:
                    ...
                    print(f"{Fore.RED}Update failed: {result}{Fore.RESET}")
                    time.sleep(2)
        else:
            print(f"{Fore.GREEN}You are running the latest version (v{CLIENT_VERSION}){Fore.RESET}")
            time.sleep(1)
    except Exception as e:
        print(f"{Fore.YELLOW}Could not check for updates: {e}{Fore.RESET}")
        time.sleep(5)
